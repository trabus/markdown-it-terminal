...
module.exports = function(md, styleOptions) {
  ...
  md.renderer.rules.code_block = function (tokens, idx /*, options, env */) {
    var e = styleOptions.unescape ? unescapeEntities : escapeHtml;
  ...
  md.renderer.rules.fence = function (tokens, idx, options /*, env, self*/) {
    ...
    var e = styleOptions.unescape ? unescapeEntities : escapeHtml;
    if (token.params) {
      langName = e(unescapeAll(token.params.split(/\s+/g)[0]));
    }

    if (options.highlight && (langName === 'javascript' || langName === 'js')) {
      highlighted = cardinal.highlight(token.content, langName) || e(token.content);
    } else {
  ...
  md.renderer.rules.paragraph_open = function (tokens, idx /*, options, env */) {
    return tokens[idx].tight ? '' : styleOptions.paragraph.open;
  };
  ...
  md.renderer.rules.image = function (tokens, idx, options, env, self) {
    var e = options.unescape ? unescapeEntities : escapeHtml;
    var src = ' src="' + e(tokens[idx].src) + '"';
    var title = tokens[idx].title ? (' title="' + e(tokens[idx].title) + '"') : '';
